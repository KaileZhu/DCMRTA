"""Attention-based neural network (Transformer encoder-decoder) for DCMRTA."""
...
from dcmrta.config import AGENT_INPUT_DIM, TASK_INPUT_DIM, EMBEDDING_DIM
...
def get_attn_pad_mask(seq_q, seq_k):
    ...
    pad_attn_mask_k = seq_q.eq(-1).all(2).data.eq(1).unsqueeze(1)  # batch_size x 1 x len_q
    pad_attn_mask_q = seq_k.eq(-1).all(2).data.eq(1).unsqueeze(1)  # batch_size x 1 x len_k
...
class SingleHeadAttention(nn.Module):
    ...
    def forward(self, q, h=None, mask=None):
        ...
        :param mask: mask (batch_size, n_query, graph_size). 1 = attention NOT possible.
        """
        if h is None:
            h = q
...
class AttentionNet(nn.Module):
    """Attention-based policy network for multi-robot task allocation."""
...
if __name__ == '__main__':
    model = AttentionNet(AGENT_INPUT_DIM, TASK_INPUT_DIM, EMBEDDING_DIM)
    node_inputs = torch.rand((128, 10, AGENT_INPUT_DIM))
    task_inputs = torch.rand((128, 20, TASK_INPUT_DIM))
    mask = torch.zeros(128, 1, 20, dtype=torch.bool)
    logp_list = model(task_inputs, node_inputs, mask)
    print(f"Output shape: {logp_list.size()}")
